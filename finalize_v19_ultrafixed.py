...
output_path = 'Wayfair_TV_Stands_FINAL_V20_ZeroErrors.xlsx'
...
for i, p in enumerate(products_data[:15]):
    ...
    mapping = {
        ...
        "Color / Finish": "White", # Hardcoding to "White" based on latest feedback
        "Pieces Included": "1",
        "Commercial Warranty": "No",
        "Number of People Needed": 2, # For Assembly
        "Estimated Time": 60, # For Assembly
        "Tools Needed": VALID_VALUES["Tools Needed"][random.randint(0, len(VALID_VALUES["Tools Needed"])-1)] if VALID_VALUES["Tools Needed"] and len(VALID_VALUES["Tools Needed"]) > 0 else "Screwdriver; Wrench" # Tools needed for assembly
    }
...
wb.save(output_path)
print(f"Final V19 saved to {output_path}")
