...
output_path = 'Wayfair_TV_Stands_FINAL_V16_Bulletproof.xlsx'
...
VALID_VALUES = {}
# Headers for Valid Values are in row 1
    ...
    if header_name:
        ...
        for r in range(2, ws_valid_values.max_row + 1):
            ...
            if val:
                ...
                break # Stop if we hit an empty cell in a valid value list
        VALID_VALUES[header_name] = values

# Fallback for some values if not found in template (should be rare)
...
# Product Database (with updated UPC logic and image links)
products_data = [
...
for i in range(1, 7):
    products_data.append({
        ...
        "upc": f"840134{random.randint(100000,999999)}", # Generate realistic 12-digit UPC
        "price": 500 + (i*75),
...
for i, p in enumerate(products_data[:15]):
    ...
    mapping = {
        ...
        "Marketing Copy": f"Elevate your living space with the {p['name']}. A perfect blend of modern style, robust construction, and versatile storage. Designed for enduring elegance and daily convenience.",
        "Feature Bullet 1": "Sleek, contemporary design for modern interiors",
        "Feature Bullet 2": "Durable construction with high-quality materials",
        "Feature Bullet 3": "Ample storage with smooth-gliding drawers/cabinets",
        "Feature Bullet 4": "Integrated wire management for a clutter-free look",
        "Feature Bullet 5": "Easy assembly with all hardware included",
        
        # Fill using VALID_VALUES and smart logic
        "Lead Time": VALID_VALUES["Lead Time"][0], # Default to 0 days
        "Replacement Lead Time": VALID_VALUES["Lead Time"][random.randint(1, len(VALID_VALUES["Lead Time"])-1)], # Pick a replacement lead time
        ...
        "Color / Finish": p['color'] if 'color' in p else VALID_VALUES["Color / Finish"][0], # Use product color or default
        "Pieces Included": "1",
    ...
    for col_idx in required_cols:
        ...
        if cell.value is None or str(cell.value).strip() == "" or str(cell.value).strip() == "N/A":
            ...
            else: cell.value = "Standard" # Last resort if everything else fails

wb.save(output_path)
print(f"Final V16 saved to {output_path}")
