...
source_path = '/root/.openclaw/media/inbound/file_8---b53228e8-76c8-4f2b-89ca-9f0757ca5a23.xlsx'
output_path = 'Wayfair_TV_Stands_Final_V8_DeepFill.xlsx'
...
# Base Product Info (Same as before)
products_data = [
...
for i in range(1, 7):
    products_data.append({
        "sku": f"CASA-TV-MODEL-{800+i}",
        "name": f"Luxury Modern TV Unit {800+i}",
        "upc": f"8401341008{10+i}",
        "price": 450 + (i*75),
        "img": "https://casadesignfurniture.com/wp-content/uploads/2023/12/Group-4-1.png",
        "w": 68, "d": 16, "h": 21, "weight": 85
    })
...
for i, p in enumerate(products_data[:15]):
    ...
    mapping = {
        ...
        "Carton Weight 1": p['weight'] + 12,
        "Carton Width 1": p['w'] + 5,
        "Carton Height 1": p['h'] + 5,
        "Carton Depth 1": p['d'] + 5,
        "Lead Time": 3,
        "Replacement Lead Time": 5,
        ...
        "Color / Finish": "White/Mirrored",
        "Pieces Included": "1",
        "Fuel Type": "Electric" if "FIREPLACE" in p['name'].upper() else "N/A",
        "Vent Type": "Vent Free" if "FIREPLACE" in p['name'].upper() else "N/A",
    ...
    for col_idx in required_cols:
        ...
        if cell.value is None or cell.value == "" or cell.value == "N/A":
            ...
            # Safety catch for anything missed
