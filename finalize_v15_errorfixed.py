...
output_path = 'Wayfair_TV_Stands_FINAL_V15_ErrorFixed.xlsx'
...
# Valid Values from Template (Manual Extraction from Valid Values tab or common sense)
VALID_VALUES = {
    "Ship Type": ["Small Parcel", "LTL"],
    "Material": ["Solid + Manufactured Wood", "Manufactured Wood", "Solid Wood", "Metal", "Glass"],
    "Level of Assembly": ["Full Assembly Needed", "Partial Assembly", "No Assembly Required"],
    "Country Of Manufacturer": ["United States", "China", "Vietnam"],
    "Furniture Design": ["Cabinet/Enclosed storage", "Open storage", "Floating"],
    "Color / Finish": ["White", "Black", "Brown", "Gray", "Silver", "Mirrored"],
    "Plug-In": ["Yes", "No"],
    "Fireplace Included": ["Yes", "No"],
    "Tipover Restraint Device Included": ["Yes", "No"],
    "Battery or Batteries Included": ["Yes", "No"],
    "General Certificate of Conformity (GCC)": ["Yes", "No"],
    "Composite Wood Product (CWP)": ["Yes", "No"],
    "CARB Phase II Compliant (formaldehyde emissions)": ["Yes", "No"],
    "TSCA Title VI Compliant (formaldehyde emissions)": ["Yes", "No"],
    "CANFER Compliant": ["Yes", "No"],
    "Uniform Packaging and Labeling Regulations (UPLR) Compliant": ["Yes", "No"],
    "Canada Product Restriction": ["Yes", "No"],
    "Energy Efficiency Regulations Compliant": ["Yes", "No"],
    "Safety Listing(s)": ["Not Applicable", "UL Listed"],
    "Commercial Warranty": ["Yes", "No"],
    "Reason for Restriction": ["None", "Size", "Weight"],
    "Vent Type": ["Vent Free", "Direct Vent"],
    "Fuel Type": ["Electric", "Bio Ethanol"]
}
...
for i in range(1, 7):
    products_data.append({
        "sku": f"CASA-TV-NEW-{300+i}",
        "name": f"Contemporary TV Console Model {300+i}",
        "upc": f"8401341003{50+i}",
        "price": 570 + (i*50),
...
for i, p in enumerate(products_data[:15]):
    ...
    mapping = {
        ...
        "TV Stand Overall Height - Top to Bottom": p['h'], # Added this for consistency
        ...
        "Ship Type": VALID_VALUES["Ship Type"][0], # Small Parcel
        "Country Of Manufacturer": VALID_VALUES["Country Of Manufacturer"][0], # United States
        "Warning Required": "No",
        "Product Type": "TV Stand",
        "Furniture Design": VALID_VALUES["Furniture Design"][0], # Cabinet/Enclosed storage
        "Material": VALID_VALUES["Material"][0], # Solid + Manufactured Wood
        ...
        "Level of Assembly": VALID_VALUES["Level of Assembly"][0], # Full Assembly Needed
        ...
        "Safety Listing(s)": VALID_VALUES["Safety Listing(s)"][0], # Not Applicable
        "Color / Finish": "White", # General Color
        "Pieces Included": "1",
    ...
    # Final sweep for Required cells, ensuring no N/A or blanks
    for col_idx in required_cols:
        ...
        if cell.value is None or str(cell.value).strip() == "" or str(cell.value).strip() == "N/A":
            ...
            if "Included" in h_name: cell.value = "No" # Default to No if not explicitly Yes
            elif "Required" in h_name: cell.value = "No"
            elif "Compliant" in h_name: cell.value = "No"
            elif "Number" in h_name: cell.value = 1
            elif "Material" in h_name: cell.value = VALID_VALUES["Material"][0]
            elif "Finish" in h_name: cell.value = VALID_VALUES["Color / Finish"][0]
            elif "Color" in h_name: cell.value = VALID_VALUES["Color / Finish"][0]
            elif "Type" in h_name and "Ship" not in h_name: cell.value = "Standard"
            elif "Design" in h_name: cell.value = VALID_VALUES["Furniture Design"][0]
            elif "Assembly" in h_name: cell.value = VALID_VALUES["Level of Assembly"][0]
...
wb.save(output_path)
print(f"Final V15 saved to {output_path}")
